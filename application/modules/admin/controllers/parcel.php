...
class parcel extends MY_Controller {
	...
	{
		$data['konten'] = $this->load->view('parcel/tambah', NULL, TRUE);
	...
	{
		//$item = 
		$data['konten'] = $this->load->view('parcel/daftar', NULL, TRUE);
		$this->load->view ('main', $data);
	}
	...
	{
		$data['konten'] = $this->load->view('parcel/ubah', NULL, TRUE);
		$this->load->view ('main', $data);
	}
	...
	public function simpan_ubah() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data['judul'] 		= $this->input->post("judul");
			$data['isi'] = $this->input->post("isi_parcel"); 
			$data['status'] = $this->input->post("status"); 
			$id 			    = $this->input->post("id");
			
			$get_old_file = $this->db->query("SELECT * FROM parcel where id='$id'");
			...
			//$data['kategori_id'] = $this->input->post("kategori");
			$this->db->update("parcel", $data, "id = " . $id);
	...
	public function hapus() {      
		$key = $_REQUEST['key'];
		$this->db->delete('parcel', array('id' => $key));
		echo '<div class="alert alert-success">Parcel sudah dihapus.</div>';    
	}
}
