...
class galery extends MY_Controller {
	...
	{
		$data['konten'] = $this->load->view('galery/tambah', NULL, TRUE);
	...
	{
		$item = 
		$data['konten'] = $this->load->view('galery/daftar', NULL, TRUE);
		$this->load->view ('main', $data);
	}
	...
	{
		$data['konten'] = $this->load->view('galery/ubah', NULL, TRUE);
		$this->load->view ('main', $data);
	}
	...
	public function simpan_ubah() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$get_old_file = $this->db->query("SELECT * FROM galery where id='$id'");
			...
			$this->db->update("galery", $data, "id = " . $id);
	...
	public function hapus() {      
		$key = $_REQUEST['key'];
		$this->db->delete('galery', array('id' => $key));
